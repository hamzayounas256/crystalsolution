...
export default function MonthlyPurchaseComparison() {
	...
	const comapnyname = "NASIR TRADERS";
	// const comapnyname = organisation.description;

	function fetchMonthlyPurchaseComparison() {
		const apiMainUrl = apiLinks + "/MonthlyPurchaseComparison.php";
		setIsLoading(true);
	...
	const exportPDFHandler = async () => {
		...
		const columnWidths = [
			27, 68, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 17,
		];
		...
		const handlePagination = () => {
			...
			while (currentPageIndex * rowsPerPage < rows.length) {
				...
				startY += 7;
				addTitle(`Monthly Purchase Comparison`, "", "", pageNumber, startY, 14);
				startY += 13;
		...
		handlePagination();
		doc.save("MonthlyPurchaseComparison.pdf");
	...
	const handleDownloadCSV = async () => {
		...
		worksheet.addRow([]);
		[comapnyname, `Monthly Purchase Comparison`].forEach((title, index) => {
		...
		[20, 45, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 12].forEach(
			(width, index) => {
		...
		saveAs(blob, "MonthlyPurchaseComparison.xlsx");
	};
	...
	useHotkeys("s", fetchMonthlyPurchaseComparison);
	...
	const handleStoreEnter = (e) => {
		if (e.key === "Enter" && !menuStoreIsOpen) {
			e.preventDefault();
			focusNextElement(storeRef, categoryRef);
		}
	};
	...
	return (
		<>
			...
			<div style={contentStyle}>
				...
				>
					<NavComponent textdata="Monthly Purchase Comparison" />
						...
						>
							...
							>
								...
								>
									...
									>
										<td className="border-dark" style={zeroColWidth}>
											Code
										</td>
												...
												return (
													...
													>
														...
														<td
															...
															title={item.Description} // Add this to show the full text on hover
														>
															<div
																style={{
																	maxWidth: "130px",
					...
					>
						...
						<SingleButton
							...
							text="Select"
							ref={selectButtonRef}
							onClick={fetchMonthlyPurchaseComparison}
							style={{ backgroundColor: "#186DB7", width: "120px" }}
							onFocus={(e) => (e.currentTarget.style.border = "2px solid red")}
							onBlur={(e) =>
								(e.currentTarget.style.border = `1px solid ${fontcolor}`)
