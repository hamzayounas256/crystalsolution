...
export default function SupplierActivity() {
	...
	function fetchSupplierActivity() {
		const fromDateElement = document.getElementById("fromdatevalidation");
		const toDateElement = document.getElementById("todatevalidation");
		...
		const apiUrl = apiLinks + "/SupplierActivity.php";
		setIsLoading(true);
		...
		axios
			...
			.then((response) => {
				...
				setTotalPayment(response.data["Total Payment"]);
	...
	const exportPDFHandler = () => {
		...
		const columnWidths = [18, 65, 20, 20, 20, 20, 20];
		...
		const handlePagination = () => {
			...
			while (currentPageIndex * rowsPerPage < rows.length) {
				...
				startY += 7;
				addTitle(
					`Supplier Activity From: ${fromInputDate} To: ${toInputDate}`,
					"",
					"",
					pageNumber,
					startY,
	...
	const handleDownloadCSV = async () => {
		...
		saveAs(blob, "SupplierActivity.xlsx");
	};
	...
	useHotkeys("s", fetchSupplierActivity);
	...
	return (
		<>
			...
			<div style={contentStyle}>
				...
				>
					<NavComponent textdata="Supplier Activity" />
					<div
						className="row"
					...
					>
						...
						>
							<span className="mobileledger_total">{totalPayment}</span>
						</div>
						<div
							style={{
								...sixthColWidth,
						...
						>
							<span className="mobileledger_total">{totalOther}</span>
						</div>
						<div
							style={{
								...seventhColWidth,
