...
export default function ItemMonthlySaleComparison() {
	...
	function fetchItemMonthlySaleComparison() {
		const apiMainUrl = apiLinks + "/ItemMonthlySaleComparison.php";
		setIsLoading(true);
	...
	const exportPDFHandler = () => {
		...
		const handlePagination = () => {
			...
			while (currentPageIndex * rowsPerPage < rows.length) {
				...
				startY += 7;
				addTitle(
					`Item Monthly Sale Comparison Report`,
					"",
					"",
					pageNumber,
					startY,
				...
				const labelXMiddleWord = doc.internal.pageSize.width - totalWidth + 120;
				const labelXMiddleTerm = doc.internal.pageSize.width - totalWidth + 135;
		...
		handlePagination();
		doc.save(`ItemMonthlySaleComparisonReport.pdf`);
	...
	const handleDownloadCSV = async () => {
		...
		worksheet.addRow([]);
		[comapnyname, `Item Monthly Sale Comparison Report`].forEach(
			(title, index) => {
		...
		worksheet
			.addRow([
				`Company: ${companyTypeDataValue ? companyTypeDataValue.label : "ALL"}`,
				"",
				"",
				"",
				"",
			...
			.eachCell((cell, colNumber) => {
				if (colNumber === 1 || colNumber === 7 || colNumber === 13) {
					cell.font = {
						bold: true,
		...
		saveAs(blob, `ItemMonthlySaleComparisonReport.xlsx`);
	};
	...
	useHotkeys("s", fetchItemMonthlySaleComparison);
	...
	const handleCompanyEnter = (e) => {
		if (e.key === "Enter" && !menuCompanyIsOpen) {
			e.preventDefault();
			focusNextElement(companyRef, typeRef);
		}
	};
	...
	return (
		<>
			...
			<div style={contentStyle}>
				...
				>
					<NavComponent textdata="Item Monthly Sale Comparison Report" />
					...
					>
						...
						<SingleButton
							...
							text="Select"
							ref={selectButtonRef}
							onClick={fetchItemMonthlySaleComparison}
							style={{ backgroundColor: "#186DB7", width: "120px" }}
							onFocus={(e) => (e.currentTarget.style.border = "2px solid red")}
							onBlur={(e) =>
								(e.currentTarget.style.border = `1px solid ${fontcolor}`)
