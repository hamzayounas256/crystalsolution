...
export default function EmployeeSaleSummary() {
	...
	function fetchEmployeeSaleSummary() {
		const fromDateElement = document.getElementById("fromdatevalidation");
		const toDateElement = document.getElementById("todatevalidation");
		...
		const apiMainUrl = apiLinks + "/EmployeeSaleSummary.php";
		setIsLoading(true);
	...
	const exportPDFHandler = () => {
		...
		const handlePagination = () => {
			...
			while (currentPageIndex * rowsPerPage < rows.length) {
				...
				startY += 7;
				addTitle(
					`Employee Sale Summary From: ${fromInputDate} To: ${toInputDate}`,
					"",
					"",
					pageNumber,
					startY,
		...
		handlePagination();
		doc.save("EmployeeSaleSummary.pdf");
	...
	const handleDownloadCSV = async () => {
		...
		saveAs(blob, "EmployeeSaleSummary.xlsx");
	};
	...
	useHotkeys("s", fetchEmployeeSaleSummary);
	...
	return (
		<>
			...
			<div style={contentStyle}>
				...
				>
					<NavComponent textdata="Employee Sale Summary" />
					...
					>
						...
						<SingleButton
							...
							text="Select"
							ref={selectButtonRef}
							onClick={fetchEmployeeSaleSummary}
							style={{ backgroundColor: "#186DB7", width: "120px" }}
							onFocus={(e) => (e.currentTarget.style.border = "2px solid red")}
							onBlur={(e) =>
								(e.currentTarget.style.border = `1px solid ${fontcolor}`)
