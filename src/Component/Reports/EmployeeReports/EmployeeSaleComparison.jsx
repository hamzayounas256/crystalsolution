...
export default function EmployeeSaleComparison() {
	...
	function fetchEmployeeSaleComparison() {
		const fromDateElement = document.getElementById("fromdatevalidation");
		const toDateElement = document.getElementById("todatevalidation");
		...
		const apiMainUrl = apiLinks + "/EmployeeSaleComparison.php";
		setIsLoading(true);
	...
	const exportPDFHandler = () => {
		...
		const columnWidths = [12, 80, 12, 20];
		...
		const handlePagination = () => {
			...
			while (currentPageIndex * rowsPerPage < rows.length) {
				...
				startY += 7;
				addTitle(
					`Employee Sale Comparison From: ${fromInputDate} To: ${toInputDate}`,
					"",
					"",
					pageNumber,
					startY,
		...
		handlePagination();
		doc.save("EmployeeSaleComparison.pdf");
	...
	const handleDownloadCSV = async () => {
		...
		tableData.forEach((item) => {
			worksheet.addRow([item.code, item.Employee, item.Qnty, item["Amount"]]);
		});
		const totalRow = worksheet.addRow(["", "Total", totalQnty, totalAmount]);
		totalRow.eachCell((cell) => {
			cell.font = { bold: true };
		});
		[9, 45, 9, 15].forEach((width, index) => {
		...
		saveAs(blob, "EmployeeSaleComparison.xlsx");
	};
	...
	useHotkeys("s", fetchEmployeeSaleComparison);
	...
	return (
		<>
			...
			<div style={contentStyle}>
				...
				>
					<NavComponent textdata="Employee Sale Comparison" />
					...
					>
						...
						<SingleButton
							...
							text="Select"
							ref={selectButtonRef}
							onClick={fetchEmployeeSaleComparison}
							style={{ backgroundColor: "#186DB7", width: "120px" }}
							onFocus={(e) => (e.currentTarget.style.border = "2px solid red")}
							onBlur={(e) =>
								(e.currentTarget.style.border = `1px solid ${fontcolor}`)
