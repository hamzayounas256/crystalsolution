...
export default function EmployeeCommissionSummary() {
	...
	const [commQuery, setCommQuery] = useState(10);
	...
	function fetchEmployeeCommissionSummary() {
		...
		const apiMainUrl = apiLinks + "/EmployeeCommissionSummary.php";
		setIsLoading(true);
		...
		axios
			...
			.then((response) => {
				...
				setTotalComm(response.data["Total Comm"]);
	...
	const exportPDFHandler = () => {
		...
		const columnWidths = [25, 75, 18, 10, 18, 18, 18];
		...
		const handlePagination = () => {
			...
			while (currentPageIndex * rowsPerPage < rows.length) {
				...
				startY += 7;
				addTitle(
					`Employee Commission Summary From: ${fromInputDate} To: ${toInputDate}`,
					"",
					"",
					pageNumber,
					startY,
		...
		handlePagination();
		doc.save("EmployeeCommissionSummary.pdf");
	...
	const handleDownloadCSV = async () => {
		...
		[18, 47, 12, 7, 12, 12, 12].forEach((width, index) => {
		...
		saveAs(blob, "EmployeeCommissionSummary.xlsx");
	};
	...
	useHotkeys("s", fetchEmployeeCommissionSummary);
	...
	const handleSaleEnter = (e) => {
		if (e.key === "Enter") {
			e.preventDefault();
			focusNextElement(saleRef, commRef);
		}
	};
	...
	const handleCommEnter = (e) => {
		if (e.key === "Enter") {
			e.preventDefault();
			focusNextElement(commRef, searchRef);
		}
	};
	...
	return (
		<>
			...
			<div style={contentStyle}>
				...
				>
					<NavComponent textdata="Employee Commission Summary" />
								...
								<div>
									<input
										...
										onChange={(e) => {
											...
											if (value >= 1 && value <= 99) {
												setCommQuery(value); // Update state only if within bounds
											} else if (value === 0 || value > 99) {
												setCommQuery(""); // Clear input if out of bounds
											}
										}}
									/>
								</div>
					...
					>
						...
						>
							<span className="mobileledger_total">{totalComm}</span>
						</div>
					</div>
					{/* Action Buttons */}
					<div
					...
					>
						...
						<SingleButton
							...
							text="Select"
							ref={selectButtonRef}
							onClick={fetchEmployeeCommissionSummary}
							style={{ backgroundColor: "#186DB7", width: "120px" }}
							onFocus={(e) => (e.currentTarget.style.border = "2px solid red")}
							onBlur={(e) =>
								(e.currentTarget.style.border = `1px solid ${fontcolor}`)
